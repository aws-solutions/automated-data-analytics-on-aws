...
import { DOCS_DIR, OPENAPI_SPEC_FILE, PACKAGE_ROOT } from './constants';
...
export async function generateDocs(generator: string) {
  console.info(`generating ${generator} docs`);
  const dir = path.join(DOCS_DIR, generator);
  ...
  /* eslint-disable */
	...
	console.info(`generated ${generator} docs:`, dir)
	/* eslint-enable */
}
...
export async function main() {
  ...
  for (const generator of DOC_GENERATORS) {
    await generateDocs(generator);
  }
}
...
if (process.argv[1] === __filename) {
  main()
    .then(() => console.log('generate openapi docs complete'))
    .catch((error) => {
      console.error(error);
      process.exit(1);
    });
