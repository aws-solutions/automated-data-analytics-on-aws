...
import { GOOGLE_SERVICE_ACCOUNT_JSON_KEYS, GoogleServiceAccountAuth } from '@ada/common';
...
type TFormData = Required<Omit<GoogleServiceAccountAuth, 'privateKeySecretName'>>
...
function extractGoogleServiceAccountDetails(details: GoogleServiceAccountAuth | string): TFormData {
  try {
    ...
    return pick(details, GOOGLE_SERVICE_ACCOUNT_JSON_KEYS) as TFormData;
  } catch (error: any) {
    console.warn(error);
    throw error;
  }
...
const GOOGLE_AUTH_VALIDATOR = JSONSCHEMA_VALIDATOR({ schema: GoogleServiceAccountAuth })
    ...
    fields: [
      ...
      },
      {
        ...
        fields: ([
          ...
          const fieldSchema = GoogleServiceAccountAuth.properties![field.name.split('.')[1]];
...
interface AuthField extends Omit<Field, 'name'> {
  name: `sourceDetails.${keyof GoogleServiceAccountAuth}`
}
