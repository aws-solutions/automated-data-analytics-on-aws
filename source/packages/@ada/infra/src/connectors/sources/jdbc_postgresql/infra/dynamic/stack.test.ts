...
import { PostgreSQLSourceStack } from './stack';
...
describe('postgresql-source-stack', () => {
  it(`should create resources with trigger type ${DataProductUpdateTriggerType.ON_DEMAND}`, () => {
    ...
    expect(() => Template.fromStack(new PostgreSQLSourceStack(app, 'test-db-stack', props))).not.toThrow();
  });
  ...
  it('unsupported trigger type should throw an error', () => {
    ...
    expect(() => Template.fromStack(new PostgreSQLSourceStack(app, 'test-db-stack', props))).toThrow();
  });
});

describe('stack/synthesizer/postgresql', () => {
  it('snapshot', async () => {
