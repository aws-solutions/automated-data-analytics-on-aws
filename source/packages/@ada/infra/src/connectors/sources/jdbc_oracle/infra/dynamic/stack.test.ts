...
import { OracleSourceStack } from './stack';
...
describe('oracle-source-stack', () => {
  it(`should create resources with trigger type ${DataProductUpdateTriggerType.ON_DEMAND}`, () => {
    ...
    expect(() => Template.fromStack(new OracleSourceStack(app, 'test-oracle-stack', props))).not.toThrow();
  });
  ...
  it('unsupported trigger type should throw an error', () => {
    ...
    expect(() => Template.fromStack(new OracleSourceStack(app, 'test-oracle-stack', props))).toThrow();
  });
});

describe('stack/synthesizer/oracle', () => {
  it('snapshot', async () => {
