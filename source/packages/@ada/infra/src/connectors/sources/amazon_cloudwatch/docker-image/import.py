...
class CloudWatchImport():
    ...
    def __init__(self, 
                 ...
                 domain_id: str,
                 trigger_type: str,
                 cross_account_access_role: str) -> None:

        if cw_log_name is None:
    ...
    def _get_logs(self, since: datetime, until: datetime) -> pd.DataFrame:
        ...
        if self._sts_client is not None:
            ...
            df = wr.cloudwatch.read_logs(
                ...
                start_time=since,
                end_time=until,
                boto3_session=self._assume_pull_data_sample_role_session(self._cross_account_access_role)
            )
        else:
            df = wr.cloudwatch.read_logs(
                log_group_names=[self._cw_log_name],
...
cwi = CloudWatchImport(CW_LOG_GROUP_NAME,
                       QUERY,
                       SINCE,
                       UNTIL,
