...
import { MSSQLServerSourceStack } from './stack';
...
describe('db-source-stack', () => {
  it(`should create resources with trigger type ${DataProductUpdateTriggerType.ON_DEMAND}`, () => {
    ...
    expect(() => Template.fromStack(new MSSQLServerSourceStack(app, 'test-db-stack', props))).not.toThrow();
  });
  ...
  it('unsupported trigger type should throw an error', () => {
    ...
    expect(() => Template.fromStack(new MSSQLServerSourceStack(app, 'test-db-stack', props))).toThrow();
  });
});

describe('stack/synthesizer/sqlserver', () => {
  it('snapshot', async () => {
