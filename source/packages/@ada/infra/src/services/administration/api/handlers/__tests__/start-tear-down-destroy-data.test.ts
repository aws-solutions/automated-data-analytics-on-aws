...
import { handler } from '../start-tear-down-destroy-data';
...
describe('start-tear-down-destroy-data', () => {
  it('should invoke the tear down lambda in destroy mode', async () => {
    ...
    expect(JSON.parse(response.body)).toEqual(
      expect.objectContaining({
        ...
        mode: TearDownMode.DESTROY_DATA,
    ...
    expect(mockInvokeAsync).toHaveBeenCalledWith({
      ...
      InvokeArgs: JSON.stringify({ mode: TearDownMode.DESTROY_DATA }),
    });
  });
});
