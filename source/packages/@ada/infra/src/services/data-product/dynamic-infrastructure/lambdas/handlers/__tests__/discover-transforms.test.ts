...
import { handler } from '../discover-transforms';

describe('discover-transforms', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  const discoverTransforms = async (classifications: string[], transformJobs: TransformJob[]) =>
    handler(
      {
        Payload: {
          databaseName: 'dbName',
  ...
  it('should set the current transform', async () => {
    const result = await discoverTransforms(
      ['json'],
  ...
  it('should handle an empty list of transforms', async () => {
    const result = await discoverTransforms(['json'], []);
  ...
  it('should throw an error if there are multiple tables with different classifications', async () => {
    await expect(async () => await discoverTransforms(['json', 'csv'], [makeTransform('custom-1')])).rejects.toThrow();
  });
