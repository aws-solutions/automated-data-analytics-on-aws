...
export type KinesisSourceStackProps = DynamicInfrastructureStackBaseProps;

/**
 * Stack for dynamic infrastructure for an kinesis source'd data product
 */
export class KinesisSourceStack extends DynamicInfrastructureStackBase<KinesisSourceStackProps> {
  ...
  protected createExternalFacingRoleInlinePolicyStatements(props: KinesisSourceStackProps): {
    [key: string]: PolicyDocument;
  } {
    const { dataProduct } = props;
    const { sourceDetails } = dataProduct;
    const sourceStream = sourceDetails as SourceDetailsKinesis;
  ...
  protected createDataSourceInfrastructureAndStateMachine(props: KinesisSourceStackProps): IStateMachine {
    const { dataProduct } = props;
    const { sourceDetails } = dataProduct;
    const sourceStream = sourceDetails as SourceDetailsKinesis;
  ...
  protected createAutomaticDataUpdateTriggerRule(_props: KinesisSourceStackProps): Rule {
    const { dataBucket } = this.staticInfrastructureReferences;
