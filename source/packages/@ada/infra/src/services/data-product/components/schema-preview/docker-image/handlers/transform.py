...
def load_transform_template(name):
    with open('/glue/transform-templates/{}.template.py'.format(name), 'r') as f:
        return f.read()


APPLY_TRANSFORM_TEMPLATE = load_transform_template('apply_transform')
DEFINE_TRANSFORM_WRAPPER_TEMPLATE = load_transform_template(
    'define_transform_wrapper')
EXECUTE_ALL_TRANSFORMS_TEMPLATE = load_transform_template(
    'execute_all_transforms')
